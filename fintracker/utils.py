...
def insert_df_to_duckdb(conn: duckdb.DuckDBPyConnection, dataframe: pd.DataFrame, table_name: str, col_select: str,
                        dedup: str = ""):
    if dataframe is None or len(dataframe) == 0:
        return
    cursor = conn.cursor()
    view_name = "temp_df_" + ''.join(random.choice(string.ascii_uppercase) for _ in range(10))
    cursor.register(view_name, dataframe)
    stmt = f"insert into {table_name} (select {col_select} from {view_name} {dedup}) "
    logging.info(f"Inserting {len(dataframe)} rows to {table_name} using statement: {stmt}")
    cursor.execute(stmt)
    cursor.unregister(view_name)
    cursor.commit()


def missing_timerange(conn: duckdb.DuckDBPyConnection):
    query = f"""
        select
        ...
        ) as h on h.ticker_full = t.ticker_full
    """
    logging.info(query)
